...
\subsubsection{Carrier-based / convencional PWM}
...
\subsubsection{Space vector PWM (SVPWM)}
...
\subsubsection{Optimal PWM (SHE/SHM/OPP)}
...
En la implementación práctica de inversores de dos niveles, es común introducir un tiempo muerto entre el apagado de un interruptor y el encendido de su complemento para evitar el cortocircuito del bus DC por superposición de tiempos de conmutación y cargas de recuperación de los diodos. Este retardo, típicamente de algunos cientos de nanosegundos a pocos microsegundos según el semiconductor y la frecuencia de conmutación, reduce el ciclo útil efectivo y provoca una deformación del voltaje de fase dependiente del sentido de la corriente, generando distorsión armónica adicional y ondulación de torque a bajas velocidades.
...
\subsubsection{Sobremodulación}
En SPWM existe una zona lineal donde la amplitud del voltaje fundamental fase-neutro generado crece proporcionalmente con la amplitud de la modulante. Al definir el índice de modulación
...
\subsubsection{Frecuencia de portadora y de conmutación}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\bigskip
\subsection{Saturación de voltaje}

El voltaje aplicado al estator de una máquina síncrona no solo se encuentra limitado por el nivel de aislamiento de los devanados, sino principalmente por el voltaje disponible en el \textit{link-DC} que alimenta al inversor y por el tipo de modulación utilizada. En el caso de la modulación sinusoidal por ancho de pulso (SPWM), para mantenerse en el rango de modulación lineal, la magnitud máxima de la componente fundamental del voltaje fase-neutro no puede exceder la mitad del voltaje del bus DC. En espacio $dq$ esto implica un límite en la magnitud del vector de tensión:
...
Este límite define una circunferencia en el plano $dq$, dentro de la cual debe ubicarse el vector de referencia $\underline{v}_{dq}$ generado por los controladores de corriente. El cumplimiento de esta restricción es fundamental para evitar saturación en el modulador y distorsiones en la corriente de fase.

A continuación, se abarcan dos conceptos importantes en lo que respecta a la saturación del voltaje. Por un lado, se satura la totalidad del voltaje, en marco $d-q$, que se entrega a la etapa de modulación.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Saturación vectorial del voltaje}
...
Existen alternativas más simples, pero menos eficientes, para limitar el voltaje en los controladores de corriente. Una de ellas consiste en asignar un límite fijo e independiente para cada eje, de modo que la suma de ambos valores se mantenga dentro del máximo permitido por el modulador. Por ejemplo, definir

\[
v_d^{\text{max}} = 0.4\,V_s^{\text{max}}
\qquad \qquad \qquad \qquad
v_q^{\text{max}} = 0.9\,V_s^{\text{max}}
\]

Si bien este método es sencillo de implementar, tiene dos desventajas principales: (i) el vector de salida deja de mantener su dirección original, ya que los ejes se limitan de forma independiente, y (ii) el sistema desaprovecha parte del voltaje disponible, pues no se utiliza la totalidad del círculo definido por la ecuación \eqref{eqn:limite_vdq_general}. En la práctica, esto da como resultado una saturación de tipo \textit{rectangular} en el plano $dq$, en lugar de una circular. Tiene el beneficio de poder priorizar la asignación de voltaje en alguno de los ejes, lo cual no permite hacer el método de re-escalamiento vectorial.

Una opción más avanzada es la saturación dinámica con prioridad, en la cual se asigna prioridad al eje $q$ (asociado al control de torque), utilizando primero el voltaje necesario en dicho eje y limitando el eje $d$ con el voltaje restante:

\[
v_{d,\text{max}} \; = \; \sqrt{(V_s^{\text{max}})^2 - v_{q,\text{cmd}}^2}.
\]

De este modo, se preserva el esfuerzo de control del torque incluso en condiciones de saturación, sacrificando en menor medida el control de flujo ($i_d$). Esta técnica es especialmente útil durante transitorios de carga, y puede implementarse en forma suave mediante escalamiento dinámico o con una lógica de prioridad discreta.

Cualquiera sea el caso, es interesante notar que se puede permitir cierto grado de sobre-modulación al reescalar el voltaje máximo según un factor arbitrario, mayor a uno (1), pero razonable:

\begin{equation}
	V_s^{max} \; = \; k \cdot \dfrac{V_{DC}}{2}
\end{equation}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Saturación dinámica de los lazos PI}
El segundo concepto corresponde a la saturación dinámica de los reguladores PI de corriente en los ejes $d$ y $q$. En este esquema, los límites de salida de cada regulador no son fijos, sino que se ajustan continuamente en función del voltaje remanente luego de incorporar los términos de acoplamiento cruzado (fuerza contraelectromotriz).De esta forma, el voltaje total aplicado en cada eje se compone de dos partes:

\begin{equation}
v_{d|q}^{\text{tot}} \; = \; v_{d|q}^{\text{PI}} + v_{d|q}^{\text{coup}}
\end{equation}

donde $v_{d|q}^{\text{coup}}$ representa los términos de acoplamiento, ya sea en el eje directo o en cuadratura. Típicamente:

\begin{equation}
v_d^{\text{coup}} = -\omega_e L_s i_q 
\qquad \qquad \qquad \qquad
v_q^{\text{coup}} = +\omega_e L_s i_d + \omega_e \psi_m.
\end{equation}

Para asegurar que el voltaje total permanezca dentro del límite impuesto por el bus DC, se restringe la salida del PI de modo que:

\begin{equation}
	\label{eqn:condicion_saturacion_dinamica}
	|v_{d|q}^{\text{PI}}| \; \leq \; v_{d|q}^{\text{lim}} \; = \; V_{s,d|q}^{\text{max}} - |v_{d|q}^{\text{coup}}|.
\end{equation}

Esta formulación garantiza que, sin importar el método empleado para calcular $V_s^{\text{max}}$ (ya sea reasignación vectorial, saturación dinámica prioritaria o reparto fijo), el regulador PI siempre opere dentro de un rango que preserve la condición:

\begin{equation}
v_{d|q}^{\text{PI}} + v_{d|q}^{\text{coup}} \leq V_{s|dq}^{\text{max}},
\end{equation}

En términos prácticos, este enfoque evita que el término de retroalimentación (PI) intente compensar los efectos de la fuerza contraelectromotriz, lo que produciría una saturación prematura y pérdida de linealidad. Además, la interacción entre ambos ejes se mantiene controlada, ya que el acoplamiento cruzado está explícitamente considerado dentro del cálculo del margen de voltaje remanente.

El mecanismo se complementa con un esquema de \textit{anti-windup} por \textit{back-calculation}, que corrige el estado integral del PI según la diferencia entre el voltaje saturado y el solicitado:

\begin{equation}
	\Delta I_{\text{int},dq} = K_{\text{aw}} \left( v_{dq}^* - v_{dq}^{\text{PI}} - v_{dq}^{\text{coup}} \right),
\end{equation}

manteniendo la linealidad del control y evitando acoplamientos no deseados entre ejes.
